...
mem_init_mp(void)
{
	...
	// LAB 4: Your code here:
        ...
        {        
                boot_map_region(kern_pgdir, kstk, KSTKSIZE,
...
// Used by page_init
static void
page_use(physaddr_t begin, physaddr_t end)
{	
	size_t bpage, epage;
	struct PageInfo *p, *next_free;

	bpage = begin >> PGSHIFT;
	epage = ROUNDUP(end, PGSIZE) >> PGSHIFT;
...
page_init(void)
{
	...
	page_use(0, PGSIZE);
	page_use(IOPHYSMEM, EXTPHYSMEM);
	page_use(PADDR((void *)0xF0100000), PADDR(boot_alloc(0)));

        page_use(MPENTRY_PADDR, MPENTRY_PADDR + PGSIZE);
}
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_link != 0)
		panic("page_free free a page with a pp_link not NULL.");  
	if (pp->pp_ref != 0)
		panic("page_free free a page with a ref count nonzero");
...
page_remove(pde_t *pgdir, void *va)
{
	...
	page = page_lookup(pgdir, va, &ptep);
	if (page) {
		page_decref(page);
		*ptep = 0;
		tlb_invalidate(pgdir, va);
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	//panic("mmio_map_region not implemented");
        oldbase = base;
        newbase = ROUNDUP(base + size, PGSIZE);
        if (newbase > MMIOLIM)
                panic("mmio_map_region: overflow MMIO region");

        boot_map_region(kern_pgdir, oldbase, size, pa, PTE_W | PTE_PCD | PTE_PWT);
        base = newbase;
...
/*
	pde_t *pgdir = &kern_pgdir[PDX(PGSIZE)];
	...
	cprintf("pte for %x: %x\n", PGSIZE, p[PTX(PGSIZE)]);
	cprintf("pgdir %x : %x\n", PGSIZE, check_va2pa(kern_pgdir, PGSIZE));		
*/
